...
void Sha256::update(const uint8_t* data, size_t len) {
    ...
    while (len > 0) {
        size_t take = BLOCK_SIZE - bufferLen_;
...
Sha256::Digest Sha256::final() {
    // Append the 0x80 byte, then pad with zeros, then the 64-bit big-endian length.
    uint64_t totalBits = bitLen_;
    uint8_t pad = 0x80;
    update(&pad, 1);
    ...
    uint8_t lenBytes[8];
    for (int i = 0; i < 8; ++i) {
        lenBytes[7 - i] = static_cast<uint8_t>(totalBits >> (i * 8));
    }
    // update() advances bitLen_, but we already captured totalBits, so push directly.
...
std::string Sha256::hexdigest(const std::vector<uint8_t>& data) {
    Digest d = hash(data);
    std::ostringstream os;
    os << std::hex << std::setfill('0');
    for (uint8_t b : d) os << std::setw(2) << static_cast<int>(b);
    return os.str();
}
